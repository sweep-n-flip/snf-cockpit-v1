...
export function initGraphQLClient(
  ...
  if (graphqlClients[chainId][clientName]) {
    logger.warn(
      `Cliente GraphQL para chain ${chainId} com nome '${clientName}' já existe. Retornando cliente existente.`,
    )
  ...
  logger.info(
    `Cliente GraphQL para chain ${chainId} com nome '${clientName}' inicializado com endpoint: ${endpoint}`,
  )
